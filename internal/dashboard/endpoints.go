...
func RegisterHandlers(mux *http.ServeMux, appCfg *config.Config, ms *monitor.MonitorService, translator *csi18n.Translator) {
	...
	for _, ep := range appCfg.Dashboards.Endpoints {
		...
		mux.HandleFunc(path, func(w http.ResponseWriter, r *http.Request) {
			handleDashboardRequest(w, r, file, appCfg, ms, translator)
		})
	}
...
func handleDashboardRequest(w http.ResponseWriter, r *http.Request, layoutPath string, appCfg *config.Config, ms *monitor.MonitorService, translator *csi18n.Translator) {
	...
	// 5. Load localization dictionaries
	lang := layoutCfg.Lang
	if lang == "" {
		lang = "en"
	}
	dict := translator.GetResolvedLanguageDict(lang)
	...
	provider := NewTelemetryProvider(m, appCfg, dict, charts)
