...
func (ctx *RequestContext) buildMeasurementArgs(m *monitor.Measurement) map[string]interface{} {
	...
	if diff25 > 0 {
		args["trend25Icon"] = ctx.Bot.TLang("en", kIcoTrendUp)
	} else if diff25 < 0 {
		args["trend25Icon"] = ctx.Bot.TLang("en", kIcoTrendDown)
	} else {
		args["trend25Icon"] = ctx.Bot.TLang("en", kIcoTrendFlat)
	}
	args["diff25"] = diff25

	diff10 := m.PM10 - m.PM10Prev
	if diff10 > 0 {
		args["trend10Icon"] = ctx.Bot.TLang("en", kIcoTrendUp)
	} else if diff10 < 0 {
		args["trend10Icon"] = ctx.Bot.TLang("en", kIcoTrendDown)
	} else {
		args["trend10Icon"] = ctx.Bot.TLang("en", kIcoTrendFlat)
	}
	args["diff10"] = diff10

	getZoneIcon := func(v, g, y float64) string {
		if v <= g {
			return ctx.Bot.TLang("en", kIcoPmLevel1)
		}
		if v <= y {
			return ctx.Bot.TLang("en", kIcoPmLevel2)
		}
		return ctx.Bot.TLang("en", kIcoPmLevel3)
	}
