...
Расчет и зонирование AQI полностью основаны на данных (data-driven). Правила расчета, брейкпоинты, методы округления и названия для отображения задаются в файле `res/aqi.json`. По умолчанию включены следующие стандарты (выбираются пользователем в боте):
...
Вы можете добавлять или изменять стандарты в `res/aqi.json` на лету, не перекомпилируя код. Бот автоматически загрузит изменения, если включена горячая перезагрузка конфигурации (config hot-reload).
