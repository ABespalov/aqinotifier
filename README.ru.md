...
- **Задержки уведомлений (lazy)** — фильтрация частых повторяющихся уведомлений на границах зон AQI за счет задержки оповещения. Установка задержки в значение `N` требует `N` подтверждающих измерений после первоначального перехода (что означает в общей сложности `N + 1` измерений подряд). Установите `0` для мгновенных оповещений (настраивается отдельно для роста и спада)
...
### Визуальные дашборды
- **Мультиформатный рендеринг** — динамические макеты экранов парсятся на лету и экспортируются в `epd_raw` (бинарный формат для e-Ink/EPD дисплеев), `png` и `bmp`
- **Резервный экран ошибок** — высококонтрастный экран с описанием ошибки, рисуемый встроенным шрифтом `Tamzen8x16r` при сбоях БД или паниках
- **Списки контроля доступа (ACL)** — валидация IP/CIDR клиентов прямо в конфигурации макетов
- **Горячая перезагрузка макетов** — файлы разметки YAML перечитываются при каждом запросе для быстрого прототипирования
...
tgbot:
  enabled: true
  token: "ВАШ_ТОКЕН_БОТА"   # или используйте file.token
```
...
Расчет и зонирование AQI полностью основаны на данных (data-driven). Правила расчета, брейкпоинты, методы округления и названия для отображения задаются в файле `assets/aqi.json`. По умолчанию включены следующие стандарты (выбираются пользователем в боте):

| Стандарт | Зоны | Описание |
|---|---|---|
| **EU** | 1-6 (Отлично -> Экстремально плохо) | European CAQI |
| **US EPA** | 1-7 (Отлично -> Опасно) | US AQI |
| **CN** | 1-7 (Отлично -> Опасно) | China AQI |

Вы можете добавлять или изменять стандарты в `assets/aqi.json` на лету, не перекомпилируя код. Бот автоматически загрузит изменения, если включена горячая перезагрузка конфигурации (config hot-reload).
...
Все тексты интерфейса, иконки и данные AQI хранятся в директории `assets/`:

| Файл | Назначение |
|---|---|
| `assets/ru.json` | Русская локализация |
| `assets/en.json` | Английская локализация |
| `assets/ico.json` | Определения иконок (emoji-плейсхолдеры) |
| `assets/colors.json` | Цветовые маппинги |
| `assets/aqi.json` | Точки перехода AQI по стандартам |
| `assets/readme.ru.md` | [Документация по шаблонизатору](assets/README.ru.md) |
...
| Рендеринг графики | [github.com/fogleman/gg](https://github.com/fogleman/gg) |
